...
const OBERMONTEUR_FOLDERS = ['01_Monteur', '02_Obermonteur', '05_Allgemein'];

@Component({
  selector: 'app-obermonteur-landing',
  imports: [CommonModule, RouterLink, AuthedUrlPipe, EmptyStateComponent],
  templateUrl: './obermonteur-landing.component.html',
  styleUrl: './landing-sections.scss',
})
export class ObermonteurLandingComponent implements OnChanges {
  ...
  protected readonly outputs = computed<ProjectOutputFile[]>(() => {
    ...
    const filtered = all.filter((f) => OBERMONTEUR_FOLDERS.some((p) => f.path.startsWith(p + '/')));
